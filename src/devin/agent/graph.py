        ...
        if should_load:
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                if len(skill_content) > 1500:
                    skill_content = skill_content[:1500] + "\n...[truncated]"
                content += f"\n--- {filename} ---\n{skill_content}\n"
...
def build_graph(
    ...
    architect_tool_names = ["read_file", "list_directory", "web_search", "get_current_time", "file_search", "grep_search", "analyze_python_ast", "git_diff"]
    worker_tool_names = ["write_file", "edit_file_replace", "execute_command", "calculator", "get_current_time", "read_file", "file_search", "grep_search", "analyze_python_ast", "git_diff", "git_status"]
    ...
    a_query_engine = QueryEngine(architect_llm, fallback_models=fallback_chain)
    w_query_engine = QueryEngine(worker_llm, fallback_models=fallback_chain)
    ...
    def initialize_node(state: AgentState) -> dict:
        ...
        if len(project_rules_content) > 2000:
            project_rules_content = project_rules_content[:2000] + "\n\n... [RULES TRUNCATED]"

        try:
            tree = _build_smart_tree(os.getcwd(), max_depth=1)
        except Exception:
    ...
    async def architect_node(state: AgentState) -> dict:
        ...
        if messages:
            ...
            if any(marker in last_content_lower for marker in worker_failure_markers):
                return {
                    "messages": [AIMessage(content="The worker ran into an API error. Please try again.")],
        ...
        if hasattr(response, "tool_calls") and response.tool_calls:
            delegate_calls = [tc for tc in response.tool_calls if tc["name"] == "delegate_to_worker"]
            if delegate_calls:
                ...
                for m in reversed(messages):
                    if isinstance(m, HumanMessage):
                        break
                    if isinstance(m, ToolMessage) and getattr(m, "name", "") == "delegate_to_worker":
                        already_delegated = True
                        break
                
                if already_delegated:
                    logger.info("Blocked duplicate delegate_to_worker in the same turn.")
                    from langchain_core.messages import AIMessage
                    response = AIMessage(content="Tớ đã delegate task này xong rồi.")
    ...
    async def worker_node(state: AgentState) -> dict:
        ...
        if not worker_recent_msgs or not any(isinstance(m, HumanMessage) for m in worker_recent_msgs):
            worker_recent_msgs = [HumanMessage(content=f"Please execute this task:\n{instructions}")] + worker_recent_msgs
        ...
        if not worker_recent_msgs:
            from langchain_core.messages import HumanMessage
            worker_recent_msgs = [HumanMessage(content=f"Please execute this task now:\n{instructions}")]
...
def architect_should_continue(state: AgentState) -> Literal["architect_tools", "__end__"]:
    ...
    if "User DENIED" in recent_content:
        logger.info("Task denied by user — stopping")
        return "__end__"
...
def worker_should_continue(state: AgentState) -> Literal["worker_tools", "architect"]:
    ...
    for m in recent:
        ...
        if "User DENIED" in content:
            logger.info("Worker stopping — user denied operation")
            return "architect"
