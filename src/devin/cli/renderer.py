...
def strip_artifacts(content: str) -> str:
    import re
    # Remove model artifact tokens
    content = re.sub(r'<\|.*?\|>', '', content)
    content = re.sub(r'I\'m providing the completion signal.*', '', content)
...
def print_response(content: str):
    ...
    clean_content = strip_artifacts(clean_content)
...
def print_task_summary(executed_tasks: list):
    if executed_tasks:
        summary_text = f"[bold green]Xong rồi, tớ đã xử lý với {len(executed_tasks)} bước công việc.[/]"
        console.print()
        console.print(Panel(summary_text, title="[bold green]📋 Task Summary[/]", border_style="green", padding=(1, 2)))
