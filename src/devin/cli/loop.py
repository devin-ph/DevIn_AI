...
PURE_CONVERSATION = [
    "nah", "nah man", "no man", "bro", "man",
    "yeah", "yep", "nope", "ok ok", "alright",
    "lol", "haha", "omg", "wtf", "damn",
    "not really", "never mind", "nevermind",
    "i don't know", "idk", "whatever",
    "not asking", "don't ask", "stop asking",
    "that's fine", "that's cool", "sounds good",
]

def _is_simple_intent(message: str) -> bool:
    msg = message.lower().strip()

    if any(p == msg or msg.startswith(p) or msg.endswith(p) for p in PURE_CONVERSATION) or any(p in msg for p in PURE_CONVERSATION):
        if not _contains_technical_keyword(msg):
            return True

    if _contains_technical_keyword(msg):
        return False
...
def _contains_technical_keyword(message: str) -> bool:
...
def generate_checkin(daily_context: str, life_context: str, patterns: str, companion_llm=None) -> str:
    """Generate a short check-in based on long absence."""
    prompt = f"""
You are DevIn. Your friend has been away for 12+ hours. 
Generate a SHORT check-in (1-2 lines max).

Daily context (including yesterday's state):
{daily_context[:800]}

Their Life context / Pending goals:
{life_context[:800]}
...
Rules for check-in:
- Max 2 sentences
- Reference yesterday's last state or pending goals (something real)
- ONE open question at the end
- Casual tone, not assistant-like
- Never say "How can I help you today?"

Example responses:
"Yo! Yesterday you were gonna Valorant all night — made it to class okay? What's the plan today?"
"Hey, class at 12:55 today. Morning's yours — small win or full rest mode?"
...
"""
    if companion_llm:
        ...
        try:
            resp = companion_llm.invoke([HumanMessage(content=prompt)])
            return resp.content.strip().strip('"')
        except Exception:
            pass
            
    return "Hey, been a while! What's the plan today?"
...
async def _save_session_memory(memory_engine, conversation: list):
    """
    End-of-session memory save flow.
    Proposes what to save, waits for user confirmation, then writes to files.
    Zero LLM calls — extracts structured data from message history.
    """
    if memory_engine is None or not conversation:
        return

    try:
        summary = memory_engine.propose_save(conversation)
    except Exception as e:
        logger.debug(f"propose_save failed: {e}")
        return

    if not summary:
        return

    proposal_text = memory_engine.format_proposal(summary)
    console.print(f"\n{proposal_text}\n")

    try:
        answer = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: console.input("  [bold white]Save? [Y/n/skip]: [/]").strip().lower()
        )
    except (EOFError, KeyboardInterrupt):
        answer = "n"

    if answer in ("", "y", "yes", "save all", "save"):
        try:
            memory_engine.save(summary)
            console.print("  [green]✅ Memory saved.[/]\n")
        except Exception as e:
            console.print(f"  [red]Save failed: {e}[/]\n")
            logger.error(f"Memory save error: {e}")
    else:
        console.print("  [dim]Memory skipped.[/]\n")
...
async def run_cli_async():
    ...
    try:
        ...
        patterns_context = memory_engine.read_domain("patterns")
        ...
        logs = list(Path("data/logs").glob("*.jsonl"))
        if logs:
            logs.sort()
            try:
                stat_result = os.stat(logs[-1])
                last_session_time = stat_result.st_mtime
            except Exception:
                pass
                
        now_ts = datetime.now().timestamp()
        has_12h_gap = False
        if last_session_time and (now_ts - last_session_time) > 12 * 3600:
            has_12h_gap = True
        ...
        if time_ctx in {"morning", "evening", "late_night"} and should_trigger_ritual(time_ctx, rituals_done):
            ...
            import json
            daily_str = json.dumps(daily_context) if isinstance(daily_context, dict) else str(daily_context)
            checkin = generate_checkin(daily_str, life_context, patterns_context, companion_llm=companion_llm)
            console.print(f"  [cyan]{checkin}[/]\n")
        else:
    ...
    while True:
        try:
            ...
            try:
                ...
                try:
                    stream_res = await _process_stream(agent, agent_status, full_messages, executed_tasks, session_cache)
            ...
            console.print("\n  [devin.system]👋 Goodbye![/]\n")
            if memory_engine:
                await _save_session_memory(memory_engine, conversation)
                memory_engine.extract_patterns()
            await cleanup()
            return
