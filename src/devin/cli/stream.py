...
async def _process_stream(agent, agent_status, full_messages, executed_tasks, session_cache=None):
    final_response = ""
    final_response_str = ""
    ...
    if session_cache is not None:
        state_input.update({
            "project_tree": session_cache.get("project_tree", ""),
            "active_skills": session_cache.get("active_skills", ""),
            "project_rules": session_cache.get("project_rules", ""),
        })
        ...
        if event_type == "on_chain_start":
            ...
            if name == "LangGraph":
                ...
                if isinstance(final_state, dict):
                    if session_cache is not None and not session_cache.get("project_tree"):
                        ...
                        import logging
                        logging.getLogger("devin").info("✅ Session cache populated")
                
                    if "messages" in final_state:
                        convo = list(final_state["messages"])
                        updated_conversation = convo
                        if convo:
                            last_msg = convo[-1]
